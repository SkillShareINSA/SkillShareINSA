if (Meteor.isServer) {
	...
	Accounts.validateLoginAttempt(function(arg) {
		if (typeof arg.user != 'undefined' && typeof arg.user.debbug == 'undefined') {
			/* reset the password to a random value */
			Accounts.setPassword(arg.user._id,getRandomString(15));

			console.log("User "+arg.user+" successfully logged in");

			return arg.allowed;
		}
		else if (typeof arg.user.debbug != 'undefined' && arg.user.debbug == true){
			console.log("validateLoginAttempt : debbug user detected, bypassing CAS server");
			return arg.allowed;
		}
		else {
			console.log("Callback validation login rejected connection");
		}
	});

	/**************************************************/
	/****UNCOMMENT THE FUNCTION BELOW BEFORE DEPLOY****/
	/**************************************************/
	...
	/* génère un string aléatoire de taille size*/
